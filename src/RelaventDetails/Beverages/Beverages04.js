...
const Beverages = () => {
  ...
  const extracard = () => {
    let a = {};
    for (let i = 0; i < beverage04.length; i++) {
      a[beverage04[i].id] = 1;
    }
    return a;
  };
  ...
  const decrement = (id) => {
    beverage04[id].qty -= 1;
    dispatch({ type: "DEC", payload: beverage04 });
  };

  const increment = (id, stock) => {
    beverage04[id].qty += 1;
    dispatch({ type: "INC", payload: beverage04 });
  };
  const sortedProduct = () => {
    let sortedArray = beverage04;
